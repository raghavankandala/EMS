class QuickLinksController < ApplicationController
  ...
  def create
    ...
    if @quick_link.save
      redirect_to @quick_link, :notice => "Successfully created quick link."
    else
      render :action => 'new'
    end
  end
  ...
  def update
    ...
    if @quick_link.update_attributes(params[:quick_link])
      redirect_to @quick_link, :notice  => "Successfully updated quick link."
    else
      render :action => 'edit'
    end
  end
  ...
  def destroy
    @quick_link = QuickLink.find(params[:id])
    @quick_link.destroy
    redirect_to quick_links_url, :notice => "Successfully destroyed quick link."
  end
end
